...
export default class Pawn extends Piece {
    ...
    isOpponent(board, x, y) {
        return this.player !== board.getPiece(Square.at(this.location.row + x, this.location.col + y)).player;
    }

    isKing(board, x, y) {
        return board.getPiece(Square.at(this.location.row + x, this.location.col + y)) instanceof King;
    }
    ...
    getAvailableMoves(board) {
        ...
        if (this.player === Player.WHITE) {
            ...
            // left diagonal
            if (!this.isEmpty(board, 1, -1) && this.isOpponent(board, 1, -1) && !this.isKing(board, 1, -1)) {
              this.addMove(1, -1);
            }
            // right diagonal
            if (!this.isEmpty(board, 1, 1) && this.isOpponent(board, 1, 1) && !this.isKing(board, 1, 1)) {
